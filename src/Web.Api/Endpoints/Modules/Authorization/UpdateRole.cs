...
internal sealed class UpdateRole : IEndpoint
{
    ...
    {
        app.MapPut("authorization/roles/{roleId:guid}", async (
            Guid roleId,
            Request request,
            HttpContext httpContext,
            IAuditTrailService auditTrailService,
            ICommandHandler<UpdateRoleCommand, RoleCrudResponse> handler,
            CancellationToken cancellationToken) =>
        {
            UpdateRoleCommand command = request.ToCommand(roleId);
