...
public sealed record ConfirmPasswordResetCommand(string Email, string Code, string NewPassword) : ICommand<IResult>;
...
{
    protected override async Task<IResult> HandleCore(ConfirmPasswordResetCommand command, CancellationToken cancellationToken)
    {
        string email = command.Email?.Trim() ?? string.Empty;
        string code = command.Code?.Trim() ?? string.Empty;

        if (string.IsNullOrWhiteSpace(email) || string.IsNullOrWhiteSpace(code) || string.IsNullOrWhiteSpace(command.NewPassword))
        {
        ...
        string? blockedUntilText = await userManager.GetAuthenticationTokenAsync(user, PasswordResetTokens.Provider, PasswordResetTokens.BlockedUntilUtc);
        if (TryParseUtc(blockedUntilText, out DateTime blockedUntilUtc) && blockedUntilUtc > DateTime.UtcNow)
        {
            return Results.Json(new { reset = false, error = "Too many failed attempts.", blockedUntilUtc }, statusCode: StatusCodes.Status429TooManyRequests);
        }

        string? expectedCodeHash = await userManager.GetAuthenticationTokenAsync(user, PasswordResetTokens.Provider, PasswordResetTokens.CodeHash);
        string? expiryText = await userManager.GetAuthenticationTokenAsync(user, PasswordResetTokens.Provider, PasswordResetTokens.ExpiryUtc);

        if (string.IsNullOrWhiteSpace(expectedCodeHash) || string.IsNullOrWhiteSpace(expiryText) || !TryParseUtc(expiryText, out DateTime expiresAtUtc))
        {
        ...
        {
            string? failedAttemptsText = await userManager.GetAuthenticationTokenAsync(user, PasswordResetTokens.Provider, PasswordResetTokens.FailedAttempts);
            ...
            if (failedAttempts >= maxFailedAttempts)
            {
                ...
                await userManager.SetAuthenticationTokenAsync(user, PasswordResetTokens.Provider, PasswordResetTokens.BlockedUntilUtc, blockUntilUtc.ToString("O"));
                await userManager.SetAuthenticationTokenAsync(user, PasswordResetTokens.Provider, PasswordResetTokens.FailedAttempts, "0");
                return Results.Json(new { reset = false, error = "Too many failed attempts.", blockedUntilUtc = blockUntilUtc }, statusCode: StatusCodes.Status429TooManyRequests);
            }

            await userManager.SetAuthenticationTokenAsync(user, PasswordResetTokens.Provider, PasswordResetTokens.FailedAttempts, failedAttempts.ToString(CultureInfo.InvariantCulture));
            return Results.BadRequest(new { error = "Reset code is invalid." });
        }

        string resetToken = await userManager.GeneratePasswordResetTokenAsync(user);
        IdentityResult resetResult = await userManager.ResetPasswordAsync(user, resetToken, command.NewPassword);
