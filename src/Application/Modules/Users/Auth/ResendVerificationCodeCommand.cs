...
public sealed record ResendVerificationCodeCommand(string Email) : ICommand<IResult>;

internal sealed class ResendVerificationCodeCommandHandler(
    UserManager<User> userManager,
    IUserRegistrationVerificationService verificationService,
    IAuditTrailService auditTrailService) : ResultWrappingCommandHandler<ResendVerificationCodeCommand>
{
    protected override async Task<IResult> HandleCore(ResendVerificationCodeCommand command, CancellationToken cancellationToken)
    {
        ...
        if (!resendResult.Sent)
        {
            ...
            {
                return Results.Json(new { sent = false, error = "Cooldown is active.", resendAvailableAtUtc = resendResult.CooldownEndsAtUtc }, statusCode: StatusCodes.Status429TooManyRequests);
            }

            return Results.BadRequest(new { sent = false, error = "Verification code cannot be resent now." });
        }

        return Results.Ok(new { sent = true, verificationExpiresAtUtc = resendResult.VerificationExpiresAtUtc, resendAvailableAtUtc = resendResult.CooldownEndsAtUtc });
    }
}
